...
class SPQRTransfromTest < ActiveSupport::TestCase
	...
	samples = Array.new
	samples [0] = '<img src="/ece2025/cgi-bin/mimetex.exe?\hat\omega"> blue'
	...
	#  end
	 ...
	 test "pre_class" do
	 	...
	 	# output1 = SPQRTransform.new.apply(a)
	 	# assert_equal expected, output1
	 	# b = parser.parse('a<PRE class = "MATLAB">blah</pre>')
	 	# output2 = SPQRTransform.new.apply(b)
	 	# assert_equal expected, output2
	 end
	 ...
	 test "span" do
	 	parser = SPQRParser.new
	 	a = parser.parse('a<span class="comment">Uh, something.</SPAN>')
	 	p a
	 end
